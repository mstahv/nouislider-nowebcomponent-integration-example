...
import in.virit.NoSliderUsageUI;
...
public class NoUiSliderIT {
    ...
    @Test
    public void testItRenders() {
        String rootUrl = "http://localhost:" + port + "/" + NoSliderUsageUI.class.getSimpleName().toLowerCase();
        page.navigate(rootUrl);

        Locator slider = page.locator("noui-slider");
        slider.waitFor();

        // noUI-target is added while rendered, consider that as proof that rendiring works
        assertThat(slider).containsClass("noUi-target");
    }
